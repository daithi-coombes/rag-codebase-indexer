...
class VectorStore extends EventEmitter {
  ...
  constructor(config, collection, client){
    super();
   ...
   * @param {Object} config.embedOptions          - the embedder options
   * @param {string} config.embedOptions.provider - the embed provider
   * @param {Class} [VectorClient]                - default ChromaClient
   ...
   * });
   */
  static async connect(config, VectorClient = ChromaClient, EmbedderClass = null) {
    const path = config.url || 'http://localhost:8000';
    const _config = {
      batchSize: 200,
      ...config
    }
    const { collection: name, dimensions } = _config;
    ...
    const client = new VectorClient(_config);
    ...
    if (!collection) {
      collection = await client.createCollection({ name, dimensions });
    }

    const store = new VectorStore(config, collection, client);

    await store.loadEmbedder(config.embedOptions, EmbedderClass);
  ...
  async ingest(indexResult) {
    ...
    const { ids, embeddings, documents, metadatas, dimensions } =
  ...
  */
  async loadEmbeddings(embedFile) {
    ...
    for (const [index, item] of validChunks.entries()) {
      ids.push(item.id);
  ...
  async search(query, options = {}) {
    ...
    // Exact + keyword both need collection.get() — fetch once, share.
    // Client-side filter is applied after fetch since collection.get()
    // doesn't support ChromaDB where-clauses.
    let allItems = null;
    const fetchAll = async () => {
      if (!allItems) {
        allItems = await this.#collection.get({ limit: 1000 });
      }
      return allItems;
    };
  ...
  async findExactMatches(query, fetchAll, filters = {}) {
    try {
      ...
      items.documents.forEach((doc, i) => {
        ...
        if (identifierMatch) {
          // console.log('docLower: ', docLower, identifierMatch);
          score = 1.0;
        }
  ...
  async semanticSearch(query, filters = {}) {
    try {
      ...
      const raw = await this.#collection.query(queryParams);

      if (!raw.documents[0]) return [];

      // Client-side filter as belt-and-suspenders: ChromaDB's where-clause
      // handles filtering server-side, but we re-check here to stay
      // consistent with exact/keyword strategies and guard against the
      // DB returning unfiltered results.
  ...
  async keywordSearch(query, fetchAll, filters = {}) {
    ...
    try {
      const keywords = VectorStore.tokeniseQuery(query);
