...
class Ollama extends Embedder{
  ...
  constructor(config = {}, log = Logger) {
    ...
    this.config = {
      ...
      // timeout: 30000,                DEV: allow user to set timeout, if needed.
      dimensions: null,
      keepAlive: '5m',
      options: {},
      headers: {},
    ...
    // Create the official Ollama client with timeout support
    const clientOpts = {
   ...
   * @param {number} [_concurrency] - Ignored; library handles batching internally
   * @returns {Promise<number[][]>} Array of embedding vectors
   * @throws {Error} If not initialized or embedding fails
   */
  async embedBatch(texts, _concurrency = 5) {
