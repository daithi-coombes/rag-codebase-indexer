...
class Indexer extends EventEmitter {
  ...
  static async create(config) {
    ...
    await indexer.loadEmbedder(embedderOpts, config.embedderClass);
    await indexer.loadChunker(config.chunker, config.chunkerClass);
  ...
  async index(options) {
    ...
    // ── Phase: scan ───────────────────────────────────────
    this.emit('progress', { phase: 'scan', status: 'start', current: 0, total: 0, message: 'Starting scanner' });
    ...
    // ── Phase: chunk ──────────────────────────────────────
    this.emit('progress', { phase: 'chunk', status: 'start', current: 0, total: files.length, message: 'Starting chunker' });
    ...
    // ── Phase: embed ──────────────────────────────────────
    this.emit('progress', { phase: 'embed', status: 'start', current: 0, total: chunks.length, message: 'Starting embedder' });
    ...
    // ── Phase: cache ──────────────────────────────────────
    this.emit('progress', { phase: 'cache', status: 'start', current: 0, total: 1, message: 'Starting caching' });

    // TODO: use flag to either writeToCache or return embeddings in result. Default writeToCache.
